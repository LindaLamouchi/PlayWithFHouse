...
public    class   ApoGameGame   extends ApoGameModel {
	...
	private void makeLoose() {
		this.iLoose = new BufferedImage(ApoGameConstants.GAME_WIDTH, ApoGameConstants.GAME_HEIGHT, BufferedImage.TYPE_INT_RGB);
	...
	private void makeBackground() {
		...
		int w = (int)(ApoGameConstants.TILE_SIZE);
		...
		g.fillOval(startX + width + 60, height - 150 - w/2, (int)(ApoGameConstants.TILE_SIZE), (int)(ApoGameConstants.TILE_SIZE));
		g.setColor(Color.DARK_GRAY);
		g.drawOval(startX + width + 60, height - 150 - w/2, (int)(ApoGameConstants.TILE_SIZE), (int)(ApoGameConstants.TILE_SIZE));

		w = (int)(ApoGameGame.POINTS_WIDTH);
		g.setColor(Color.BLACK);
		g.fillOval(startX + width + 130, startY + 40 - w/2, (int)(ApoGameGame.POINTS_WIDTH), (int)(ApoGameGame.POINTS_WIDTH));
		g.setColor(Color.DARK_GRAY);
		g.drawOval(startX + width + 130, startY + 40 - w/2, (int)(ApoGameGame.POINTS_WIDTH), (int)(ApoGameGame.POINTS_WIDTH));
	...
	private void setEditor  (boolean visible) {
		this.getGame().setEditor(visible);
	}
	...
	public void makeLevel(int level) {
		...
		this.getGame().setButtonVisible(ApoGameConstants.BUTTON_GAME);
		...
		if (level == -2) {
			...
			this.curLevel = ApoGameLevelGeneration.getLevel(this.level);
			if (this.curLevel == null) {
				this.level = 0;
				this.curLevel = ApoGameLevelGeneration.getLevel(this.level);	
			}
	...
	public void setLevelFromEditor(ApoGameLevel level) {
		this.curLevel = level;
		this.makeLevel(-2);
	}
		...
		if (this.curLooseTime <= 0) {
			if (this.curLevel != null) {
				if (direction == KeyEvent.VK_LEFT) {
					if (this.curLevel.changeDirection(ApoGameConstants.PLAYER_DIRECTION_LEFT)) {
						this.addMove();
						this.solution += "l";
					}
				} else if (direction == KeyEvent.VK_RIGHT) {
					if (this.curLevel.changeDirection(ApoGameConstants.PLAYER_DIRECTION_RIGHT)) {
						this.addMove();
						this.solution += "r";
					}
				} else if (direction == KeyEvent.VK_UP) {
					if (this.curLevel.changeDirection(ApoGameConstants.PLAYER_DIRECTION_UP)) {
						this.addMove();
						this.solution += "u";
					}
				} else if (direction == KeyEvent.VK_DOWN) {
					if (this.curLevel.changeDirection(ApoGameConstants.PLAYER_DIRECTION_DOWN)) {
						this.addMove();
						this.solution += "d";
					}
				}
	...
	private void addMove() {
		this.moves += 1;
		this.getGame().setShouldRepaint(true);
		// this.getGame().playSound(ApoGamePanel.SOUND_POP, -1);
	}
	...
	public void mouseButtonFunction(String function) {
		if ((function.equals(ApoGameGame.MENU)) ||
			(function.equals(ApoGameGame.MENU_SCORE))) {
			if (this.level == -2) {
				setEditor(false);
			} else {
				this.getGame().setLevelChooser();
			}
		} else if (function.equals(ApoGameGame.LEFT_SORT)) {
			this.changeSort(-1);
		} else if (function.equals(ApoGameGame.RIGHT_SORT)) {
			this.changeSort(+1);
		} else if (function.equals(ApoGameGame.LEFT_LEVEL)) {
			this.changeLevel(-1);
		} else if (function.equals(ApoGameGame.RIGHT_LEVEL)) {
			this.changeLevel(+1);
		}
	}
		...
		if (this.curLooseTime > 0) {
			...
			if (!this.curLevel.think(delta)) {
				...
				for (int p = 0; p < collision.length; p++) {
					if (this.curLevel.isLayerVisible(p)) {
						for (int e = 0; e < collision[0].length; e++) {
							if (collision[p][e] == ApoGameConstants.PLAYER_DIRECTION_FINISH) {
								this.curLevel.changeLevelVisible(p);
							} else if (collision[p][e] >= 0) {
								//this.getGame().playSound(ApoGamePanel.SOUND_LOOSE, -1);
								this.makeLoose();
								this.restart();
								return;
							}
				...
				if (!this.curLevel.isVisible()) {
					//this.getGame().playSound(ApoGamePanel.SOUND_WIN, -1);
					if (this.level >= 0) {
	...
	public ArrayList<ApoGameString> getStrings() {
		return this.strings;
	}
	...
	public void setStrings(ArrayList<ApoGameString> strings) {
	...
	public void render(Graphics2D g) {
		...
		int nextH = (int)(ApoGameConstants.TILE_SIZE + 6);
		g.setFont(ApoGameGame.FONT_TEXTFIELD);
		...
		for (int i = 0; i < s.length(); i++) {
			...
			int x = 5 + (int)(65) * 7 + 5 + 60 + ApoGameConstants.TILE_SIZE/2;
			...
			if (i >= 1) {
				...
				g.setStroke(new BasicStroke(5));
				g.setColor(Color.BLACK);
				g.fillOval(x - ApoGameConstants.TILE_SIZE/2, y - h/2 - ApoGameConstants.TILE_SIZE/2, (int)(ApoGameConstants.TILE_SIZE), (int)(ApoGameConstants.TILE_SIZE));
				g.setColor(Color.DARK_GRAY);
				g.drawOval(x - ApoGameConstants.TILE_SIZE/2, y - h/2 - ApoGameConstants.TILE_SIZE/2, (int)(ApoGameConstants.TILE_SIZE), (int)(ApoGameConstants.TILE_SIZE));
				g.drawLine(x - ApoGameConstants.TILE_SIZE/2 - 5, y - h/2, x - ApoGameConstants.TILE_SIZE/2, y - h/2);
				g.setStroke(stroke);
			}
		...
		nextH = (int)(ApoGameGame.POINTS_WIDTH + 6);
		s = String.valueOf(this.moves);
		int x = 5 + (int)(65) * 7 + 5 + 130 + ApoGameGame.POINTS_WIDTH/2;
		for (int i = 0; i < s.length(); i++) {
			String p = s.substring(i, i + 1);
			...
			if (i >= 1) {
				...
				g.setStroke(new BasicStroke(5));
				g.setColor(Color.BLACK);
				g.fillOval(x - ApoGameGame.POINTS_WIDTH/2, y - h/2 - ApoGameGame.POINTS_WIDTH/2, (int)(ApoGameGame.POINTS_WIDTH), (int)(ApoGameGame.POINTS_WIDTH));
				g.setColor(Color.DARK_GRAY);
				g.drawOval(x - ApoGameGame.POINTS_WIDTH/2, y - h/2 - ApoGameGame.POINTS_WIDTH/2, (int)(ApoGameGame.POINTS_WIDTH), (int)(ApoGameGame.POINTS_WIDTH));
				g.drawLine(x, y - h/2 - ApoGameGame.POINTS_WIDTH/2 - 5, x, y - h/2 - ApoGameGame.POINTS_WIDTH/2);
				g.setStroke(stroke);
			}
			g.setColor(Color.WHITE);
			int w = g.getFontMetrics().stringWidth(p);
		...
		if (this.curLevel != null) {
			...
			if (this.curLevel.getLayer() == 1) {
				...
				g.setColor(Color.BLACK);
				g.setFont(ApoGameGame.FONT_IMAGE_HELP);
				int imageX = 15;
				int imageY = ApoGameConstants.GAME_HEIGHT - ApoGameConstants.TILE_SIZE - 20;
				if (levelGame > 3) {
					Stroke stroke = g.getStroke();
					g.setStroke(new BasicStroke(5));
					g.drawLine(10, imageY - 40, 10 + ApoGameConstants.LEVEL_WIDTH, imageY - 40);
					g.setStroke(stroke);
					String help = "help";
					int w = g.getFontMetrics().stringWidth(help);
					g.drawString(help, ApoGameConstants.LEVEL_WIDTH/2 + 10 - w/2, imageY - 15);
					String imageString = "vertical";
					g.drawString(imageString, imageX + 10, imageY + 5);
					g.drawImage(ApoGameImages.ORIGINAL_UP, imageX, imageY + 5, null);
					imageX += ApoGameImages.ORIGINAL_UP.getWidth() + 5;
					if (levelGame > 7) {
						g.drawImage(ApoGameImages.ORIGINAL_DOWN, imageX, imageY + 5, null);
						imageX += ApoGameImages.ORIGINAL_DOWN.getWidth() + 15;
						if (levelGame > 11) {
							imageString = "on / off";
							g.drawString(imageString, imageX + 10, imageY + 5);
							g.drawImage(ApoGameImages.ORIGINAL_VISIBLE_TRUE, imageX, imageY + 5, null);
							imageX += ApoGameImages.ORIGINAL_VISIBLE_TRUE.getWidth() + 5;
							if (levelGame > 16) {
								g.drawImage(ApoGameImages.ORIGINAL_VISIBLE_FALSE, imageX, imageY + 5, null);
								imageX += ApoGameImages.ORIGINAL_VISIBLE_FALSE.getWidth() + 15;
								if (levelGame > 21) {
									imageString = "horizontal";
									g.drawString(imageString, imageX + 10, imageY + 5);
									g.drawImage(ApoGameImages.ORIGINAL_LEFT, imageX, imageY + 5, null);
									imageX += ApoGameImages.ORIGINAL_LEFT.getWidth() + 5;
									if (levelGame > 24) {
										g.drawImage(ApoGameImages.ORIGINAL_RIGHT, imageX, imageY + 5, null);
										imageX += ApoGameImages.ORIGINAL_RIGHT.getWidth() + 15;
									}
									if (levelGame > 27) {
										imageString = "button";
										g.drawString(imageString, imageX + 10, imageY + 5);
										g.drawImage(ApoGameImages.ORIGINAL_STEP, imageX, imageY + 5, null);
										imageX += ApoGameImages.ORIGINAL_STEP.getWidth() + 5;
										g.drawImage(ApoGameImages.ORIGINAL_STEP_FINISH, imageX, imageY + 5, null);
										imageX += ApoGameImages.ORIGINAL_STEP_FINISH.getWidth() + 5;
									}
								}
							}
						}
		...
		if (this.curLooseTime > 0) {
			...
			g.setClip(new Rectangle2D.Double(10, 0, ApoGameConstants.LEVEL_WIDTH, ApoGameConstants.GAME_HEIGHT));
		...
		if (this.level == -3) {
			...
			final int curX = (ApoGameConstants.GAME_WIDTH - 10 - ApoGameConstants.LEVEL_WIDTH)/2 + 10 + ApoGameConstants.LEVEL_WIDTH;
			sortString(s);
			int w = g.getFontMetrics().stringWidth(s);
			final int curY = (int)(this.getGame().getButtons().get(ApoGameGame.LEFT_SORT).getY() + this.getGame().getButtons().get(ApoGameGame.LEFT_SORT).getHeight()/2 + g.getFontMetrics().getHeight()/2 - 1 * g.getFontMetrics().getDescent());
