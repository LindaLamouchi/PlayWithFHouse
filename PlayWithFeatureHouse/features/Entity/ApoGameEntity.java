...
public   class  ApoGameEntity  extends ApoGamePlayer {
	...
	public ApoGameEntity(BufferedImage pic, float x, float y, float width, float height, int fixedMovement, boolean bStartVisible) {
	...
	public void think(int delta) {
		...
		if (super.isMoving()) {
			...
			if (this.curChangeTime >= ApoGameEntity.MAX_TIME) {
				this.bShow = !this.bShow;
				this.curChangeTime -= ApoGameEntity.MAX_TIME;
			}
		}
	}
		...
		if (super.isBVisible()) {
			if (super.getIBackground() != null) {
				if ((this.bAnotherEntityOver) && (this.bShow)) {
					...
					float alpha = 1 - ((float)this.curChangeTime) / (float)ApoGameEntity.MAX_TIME;
					...
					Composite composite = g.getComposite();
					float alpha = ((float)this.curChangeTime) / (float)ApoGameEntity.MAX_TIME;
					g.setComposite(AlphaComposite.getInstance(AlphaComposite.SRC_OVER,alpha));
					g.drawImage(this.getIBackground(), (int)(this.getX() + x), (int)(this.getY() + y), null);
					g.setComposite(composite);
				} else {
