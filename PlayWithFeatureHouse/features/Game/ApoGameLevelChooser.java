...
public   class  ApoGameLevelChooser  extends ApoGameModel {
	...
	public ApoGameLevelChooser  (ApoGamePanel game) {		
		super(game);
	...
	public void init() {
		...
		if (this.buttons == null) {
			...
			for (int i = 0; i < 31; i++) {
				...
				x += 65;
				if (x + width >= ApoGameConstants.GAME_WIDTH - 20) {
					x = 20;
					y += 75;
					if ((y > 100) && (y < 200)) {
						y = 310;
		...
		if (this.iBackground == null) {
			...
			String s = ApoGameConstants.PROGRAM_NAME + " - LevelChooser";
	...
	public void mouseButtonFunction(String function) {
		if (function.equals(ApoGameLevelChooser.MENU)) {
			this.getGame().setMenu();
		}
	}
